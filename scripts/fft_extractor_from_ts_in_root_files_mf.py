...
def fastFT(timeseries):
    #returns fft, max value in fft, x_bins --> all array_like
    
    return pyfftw.interfaces.scipy_fftpack.rfft(timeseries, n=len(timeseries)), np.amax(pyfftw.interfaces.scipy_fftpack.rfft(timeseries, n=len(timeseries))), [i for i in range(0, len(timeseries))]
...
signal_window_start_from = 0
num_signal_windows = 48
noise_window_start_from = 100
num_noise_windows = 195
###########################################################################################
...
def hist_collector(root_file):
    try:
        data = []
        for i in range(signal_window_start_from, signal_window_start_from+num_signal_windows):
            hist_TS = root_file[f'histTSReal_{i}_0;1'].to_numpy()
            timeseries = generate_signal(hist_TS[0])
            fft_ts,_,__ = fastFT(timeseries)
            data.append(abs(fft_ts)/abs(_))
        return data
...
def noise_collector(root_file):
    try:
        data = []
        for i in range(noise_window_start_from, noise_window_start_from+num_noise_windows):
            hist_TS = root_file[f'histTSReal_{i}_0;1'].to_numpy()
            timeseries = generate_signal(hist_TS[0])
            fft_ts,_,__ = fastFT(timeseries)
            data.append(abs(fft_ts)/abs(_))
...
def main():
    ...
    for root_file_name in root_file_names:
        ...
        if data:
            ...
            save_label_to_csv(np.array([1 for i in range(signal_window_start_from, num_signal_windows+signal_window_start_from)])) # labeling all windows with 1's
    ...
    for root_file_name in root_file_names:
        ...
        if data:
            save_data_to_csv(np.array(data))
            save_label_to_csv(np.array([0 for i in range(noise_window_start_from, noise_window_start_from+num_noise_windows)])) # labeling all windows with 0's
            
            if checkpoint%5 == 0: print(data) # Print current root file info with label            

# function for saving data every step (step = root file). Make sure file_name already doesn't exist
def save_data_to_csv(data, file_name="89_degree_electron_tracks_20_80_1.csv"):
    data = np.array(data)
...
def save_label_to_csv(data, file_name="label_89_degree_electron_tracks_20_80_1.csv"):
    data = np.array(data)
    df = pd.DataFrame(data)
    df.to_csv(file_name, header=None, index=None, mode="a")
